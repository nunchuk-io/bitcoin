...
def key_id_to_fingerprint(keyid_str):
    fpr_rev = keyid_str[-8:]
    fpr = ''
...
class ImportMultiTest(BitcoinTestFramework):
    ...
    def run_test (self):
        ...
        pub_fpr = key_id_to_fingerprint(info['hdmasterkeyid'])
        ...
        print(pub_import_info)
        assert_equal(key_id_to_fingerprint(pub_import_info['hdmasterkeyid']), pub_fpr)
        ...
        priv_fpr = key_id_to_fingerprint(info['hdmasterkeyid'])
        result = self.nodes[0].importmulti(
            [{
        ...
        print(priv_import_info)
        assert_equal(key_id_to_fingerprint(priv_import_info['hdmasterkeyid']), priv_fpr)
        ...
        assert_equal(key_id_to_fingerprint(import_info['hdmasterkeyid']), pub_fpr)
        assert_equal(import_info['hdkeypath'], pub_keypath)
        import_info = self.nodes[0].getaddressinfo(priv_addr)
        assert_equal(key_id_to_fingerprint(import_info['hdmasterkeyid']), priv_fpr)
