...
class AddressTypeTest(BitcoinTestFramework):
    ...
    def test_descriptor(self, node, address, multisig, typ, utxo):
        """Run sanity checks on a descriptor reported by getaddressinfo."""
        info = self.nodes[node].getaddressinfo(address)
        assert('descriptor' in info)
        assert_equal(info['descriptor'], utxo['descriptor'])
        assert(self.nodes[node].validateaddress(address)['isvalid'])
        ...
        if not multisig and typ == 'legacy':
            # P2PKH
            assert_equal(info['descriptor'], "pkh(%s)" % key_descs[info['pubkey']])
        elif not multisig and typ == 'p2sh-segwit':
            # P2SH-P2WPKH
            assert_equal(info['descriptor'], "sh(wpkh(%s))" % key_descs[info['pubkey']])
        elif not multisig and typ == 'bech32':
            # P2WPKH
            assert_equal(info['descriptor'], "wpkh(%s)" % key_descs[info['pubkey']])
        elif typ == 'legacy':
            # P2SH-multisig
            assert_equal(info['descriptor'], "sh(multi(2,%s,%s))" % (key_descs[info['pubkeys'][0]], key_descs[info['pubkeys'][1]]))
        elif typ == 'p2sh-segwit':
            # P2SH-P2WSH-multisig
            assert_equal(info['descriptor'], "sh(wsh(multi(2,%s,%s)))" % (key_descs[info['embedded']['pubkeys'][0]], key_descs[info['embedded']['pubkeys'][1]]))
        elif typ == 'bech32':
            # P2WSH-multisig
            assert_equal(info['descriptor'], "wsh(multi(2,%s,%s))" % (key_descs[info['pubkeys'][0]], key_descs[info['pubkeys'][1]]))
        else:
            # Unknown type
            assert(False)
            ...
            for n, to_node in enumerate(range(from_node, from_node + 4)):
                ...
                for utxo in self.nodes[to_node].listunspent():
                    if utxo['address'] == addresses[to_node][0]:
                        found = True
                        self.test_descriptor(to_node, addresses[to_node][0], multisig, addresses[to_node][1], utxo)
                        break
                assert found
