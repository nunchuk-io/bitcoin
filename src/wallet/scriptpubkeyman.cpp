...
bool DescriptorScriptPubKeyMan::Unlock(const CKeyingMaterial& master_key_in, bool accept_no_keys)
{
    {
        LOCK(cs_desc_man);
        if (!SetCrypted())
        ...
        for (; mi != m_map_crypted_keys.end(); ++mi)
        {
            ...
            CKey key;
            if (!DecryptKey(master_key_in, crypted_secret, pubkey, key))
            {
                keyFail = true;
                break;
            }
        ...
        if (keyFail || (!keyPass && !accept_no_keys))
            return false;
        m_master_key = master_key_in;
        m_decryption_thoroughly_checked = true;
    }
    return true;
}

bool DescriptorScriptPubKeyMan::Encrypt(CKeyingMaterial& master_key_in, WalletBatch* batch)
{
    LOCK(cs_desc_man);
    if (!m_map_crypted_keys.empty() || IsCrypted()) {
        return false;
    }
    ...
    for (const KeyMap::value_type& key_in : m_map_keys)
    {
        ...
        if (!EncryptSecret(master_key_in, secret, pubkey.GetHash(), crypted_secret)) {
            return false;
        }
...
bool DescriptorScriptPubKeyMan::TopUp(unsigned int size)
{
    ...
    assert(descriptor.range_end - descriptor.range_start == descriptor.cache.size());
