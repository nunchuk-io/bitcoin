...
static UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "    \"descriptor\" : xxx,       (string, only when solvable) A descriptor for spending this output\n"
    ...
    for (const COutput& out : vecOutputs) {
        ...
        if (out.fSolvable) {
            entry.pushKV("descriptor", descriptor->ToString());
        }
...
UniValue getaddressinfo(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "  \"descriptor\" : \"desc\",        (string, optional) A descriptor for spending coins sent to this address (only when solvable)\n"
