...
DBErrors WalletBatch::LoadWallet(CWallet* pwallet)
{
    ...
    for (auto desc_key_pair : wss.m_descriptor_keys) {
        ...
        std::dynamic_pointer_cast<DescriptorScriptPubKeyMan>(spk_man)->AddKey(desc_key_pair.first.second, desc_key_pair.second);
    }
    for (auto desc_key_pair : wss.m_descriptor_crypt_keys) {
        auto spk_man = pwallet->GetScriptPubKeyMan(desc_key_pair.first.first);
        std::dynamic_pointer_cast<DescriptorScriptPubKeyMan>(spk_man)->AddCryptedKey(desc_key_pair.first.second, desc_key_pair.second.first, desc_key_pair.second.second);
    }
