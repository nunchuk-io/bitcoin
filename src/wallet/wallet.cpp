...
void CWallet::GetKeyBirthTimes(interfaces::Chain::Lock& locked_chain, std::map<CKeyID, int64_t>& mapKeyBirth) const {
    ...
    std::shared_ptr<LegacyScriptPubKeyMan> spk_man = std::dynamic_pointer_cast<LegacyScriptPubKeyMan>(m_external_spk_managers.at(OutputType::LEGACY));
...
std::shared_ptr<CWallet> CWallet::CreateWalletFromFile(interfaces::Chain& chain, const WalletLocation& location, uint64_t wallet_creation_flags)
{
    ...
    if (gArgs.GetBoolArg("-upgradewallet", false)) {
        ...
        for (bool internal : {false, true}) {
            for (OutputType t : output_types) {
                ...
                if (spk_man && !spk_man->Upgrade(prev_version, max_version, error)) {
    ...
    if (fFirstRun)
    {
        ...
        for (bool internal : {false, true}) {
            for (OutputType t : output_types) {
                auto spk_man = walletInstance->GetScriptPubKeyMan(t, internal);
                if (spk_man && spk_man->HavePrivateKeys()) {
...
std::shared_ptr<DescriptorScriptPubKeyMan> CWallet::HasWalletDescriptor(const WalletDescriptor& desc) const
{
    if (!IsWalletFlagSet(WALLET_FLAG_DESCRIPTORS)) {
        return nullptr;
    }
    ...
    for (auto spk_man_pair : m_spk_managers) {
        ...
        std::shared_ptr<DescriptorScriptPubKeyMan> spk_manager = std::dynamic_pointer_cast<DescriptorScriptPubKeyMan>(spk_man_pair.second);
...
std::shared_ptr<ScriptPubKeyMan> CWallet::AddWalletDescriptor(WalletDescriptor& desc, const FlatSigningProvider& signing_provider, const std::string& label, Optional<OutputType>& parsed_output_type)
{
    if (!IsWalletFlagSet(WALLET_FLAG_DESCRIPTORS)) {
